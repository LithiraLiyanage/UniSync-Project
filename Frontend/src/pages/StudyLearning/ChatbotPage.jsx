  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)] min-h-[600px] animate-fade-in pb-10">
      ...
      <div className="flex-1 bg-card border border-border rounded-xl shadow-sm flex flex-col overflow-hidden">
        {/* Chat Header */}
        <div className="bg-primary/5 border-b border-border p-4 flex items-center">
        ...
        {/* Chat Messages */}
        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-bg">
            ...
            {messages.map((msg) => (
              ...
              >
                <div className={`flex max-w-[80%] ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.sender === 'user' ? 'bg-gray-200 dark:bg-gray-700 text-muted ml-2' : 'bg-primary border border-primary-dark text-white mr-2 shadow-sm'}`}>
                    {msg.sender === 'user' ? <FiUser size={14} /> : <FiCpu size={14} />}
                  </div>
                  
                  <div className={`p-3.5 rounded-2xl shadow-sm ${msg.sender === 'user' ? 'bg-primary text-white rounded-tr-none' : 'bg-card border border-border text-text rounded-tl-none'}`}>
          ...
          {isTyping && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} className="flex justify-start">
              <div className="flex flex-row">
                <div className="w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center mr-2 shadow-sm"><FiCpu size={14} /></div>
        ...
        <div className="p-4 bg-card border-t border-border">
          <form onSubmit={handleSend} className="relative flex items-center">
            <input
              ...
              className="w-full pl-5 pr-14 py-3.5 bg-bg border border-border rounded-full focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary shadow-inner text-text text-sm transition-all"
            />
            <button
              type="submit"
              disabled={!input.trim() || isTyping}
              className="absolute right-2 p-2 bg-primary text-white rounded-full hover:bg-primary-dark disabled:opacity-50 transition-colors shadow-sm focus:outline-none"
            >
      ...
      <div className="w-full lg:w-80 flex flex-col gap-6 shrink-0">
          ...
          <div className="space-y-2">
            {suggestedQuestions.map((q, i) => (
              <button
                ...
                disabled={isTyping}
                className="w-full text-left p-3 rounded-lg border border-border bg-bg text-sm text-text hover:border-primary hover:bg-primary/5 transition-all focus:outline-none"
              >
                {q}
              </button>
            ))}
        ...
        <div className="bg-gradient-to-br from-primary-light to-blue-50 dark:from-primary/10 dark:to-bg border border-primary/20 rounded-xl p-5 shadow-sm">
          <h3 className="font-bold text-text mb-2 text-md">How I can help?</h3>
          <ul className="space-y-2 text-xs text-text/80 list-disc pl-4">
