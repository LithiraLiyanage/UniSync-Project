  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        {/* Type Filter */}
        <div className="flex bg-card border border-border rounded-lg p-1 overflow-x-auto w-full sm:w-auto">
          {['all', 'video', 'document', 'article', 'tool'].map(type => (
            <button
              key={type}
              onClick={() => setFilterType(type)}
              className={`px-4 py-1.5 rounded-md text-sm font-medium capitalize transition-colors ${
                filterType === type ? 'bg-amber text-white shadow-sm' : 'text-muted hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-text'
              }`}
            >
              {type}
            </button>
      ...
      {/* Resources Grid */}
      {loading ? (
        <div className="flex justify-center py-20"><div className="w-10 h-10 border-4 border-amber/20 border-t-amber rounded-full animate-spin"></div></div>
      ) : (
